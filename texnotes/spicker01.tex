\fbox{\begin{minipage}[tc]{\linewidth-2\fboxsep}
	...
	\end{tabular}
	\mbox{Modellfunktion}:
	$\displaystyle y(t) = \alpha\frac{1}{1+t} +\beta$
	\begin{dmath*}
		A = \begin{pmatrix}
			y(t_1 = 0)\\
			y(1)\\
...
\end{minipage}}
\fbox{
	\begin{minipage}[tc]{\linewidth-2\fboxsep}
...
\fbox{\begin{minipage}[tc]{\linewidth-2\fboxsep}
	...
	kann ein Teil der Matrix zu null transformiert werden. 
	...
	Damit wird \mbox{$\displaystyle Q_u \coloneqq \1_{m\times m} -2\cdot \frac{u\cdot u^T}{u^T\cdot u}$} definiert, welche $A$ so verdrehspiegelt, 
	dass alle Elemente in der ersten Spalte unterhalb der Diagonalen verschwinden.
...
\end{minipage}}
\fbox{\begin{minipage}[tc]{\linewidth-2\fboxsep}
	\textbf{Nicht Lineares Ausgleichsproblem}: \linebreak[3]
	\mbox{Daten}:
	\begin{tabular}{r|c|c|c|c}
		$t_i$ & 0 & 1 & 2 \\ 
		\hline 
	...
	\end{tabular}
	\mbox{Modellfunktion}:
	\mbox{$\displaystyle y(t) = x_1 \sin(x_2 \cdot t)$}
	\begin{dmath*}
		\phi(t, x) \hiderel{=} y(t)\\
		F_i \hiderel{=} y_i - \phi(t_i, x)\\
		F(x)
		\hiderel{=}
		\begin{pmatrix}
			2&-1\\
...
\fbox{\begin{minipage}[tc]{\linewidth-2\fboxsep}
	...
	\begin{algorithmic}[1]
		...
		\State $ y^{k+1} = \displaystyle\frac{x^{(k+1)}}{\norm{x^{(k+1)}}_2}$
		\EndFor
	\end{algorithmic}
\end{minipage}
}
\fbox{\begin{minipage}[tc]{\linewidth-2\fboxsep}
	...
	\begin{algorithmic}[1]
		...
		\State $\lambda^{(k+1)} = \displaystyle\frac{1}{{y^{(k)}}^T \cdot x^{(k)}} + \mu$
		\State $ y^{k+1} = \displaystyle\frac{x^{(k+1)}}{\norm{x^{(k+1)}}_2}$
		\EndFor
	\end{algorithmic}
\end{minipage}}
